        ...
        for s in training_queries_num:
            ...
            for li in all_li:
                ...
                for selected_features in feature_classes:
                    ...
                    input_features =  ["step", "dists","first_nn_dist", "visited_points", "nn_dist", "furthest_dist","avg_dist", "variance"] + query_dims_features
                    ...
                    # 构造保存路径（和模型同名更清晰）
                    fi_file = f"/root/DARTH-main-data/feature_importance/raet/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_删除插入前k位置数据点数量特征.csv"
                    # 保存到文件
                    os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                    feature_importances.to_csv(fi_file, index=False)
                    print(f"特征重要性保存到: {fi_file}")

                    model_file = f"/root/DARTH-main-data/predictor_models/raet/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_删除插入前k位置数据点数量特征.txt"
