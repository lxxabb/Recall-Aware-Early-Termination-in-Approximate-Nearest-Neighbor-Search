    ...
    do
        ...
        for tuple in "${tuples[@]}"; do
            ...
            if true; then
              ...
              python /root/DARTH-main/experiments/ALL_graphIndex_trainData/REM-Laet-早停统计.py \
                  ${RESULTS_DIRECTORY}/laet/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}_f${F}.txt \
                  ${RESULTS_DIRECTORY}/laet/${ds}/k${k}/statistics_tr${target_recall}.json \
                  $target_recall

              echo "从平均搜索结果文件中，所有查询点的早停统计结果statistics，写入完成：${RESULTS_DIRECTORY}/laet/${ds}/k${k}/statistics_tr${target_recall}.json"
