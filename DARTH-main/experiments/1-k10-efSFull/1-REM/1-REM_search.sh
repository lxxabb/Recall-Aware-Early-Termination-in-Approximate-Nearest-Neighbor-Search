        ...
        for tuple in "${tuples[@]}"; do
          ...
          if true; then
            ...
            python /root/DARTH-main/experiments/0-ALL_graphIndex_trainData/REM-Laet-早停统计.py \
                ${RESULTS_DIRECTORY}/REM/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                ${RESULTS_DIRECTORY}/REM/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}.json \
                $target_recall

            echo "从平均搜索结果文件中，所有查询点的统计结果statistics，写入完成：${RESULTS_DIRECTORY}/REM/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}.json"
