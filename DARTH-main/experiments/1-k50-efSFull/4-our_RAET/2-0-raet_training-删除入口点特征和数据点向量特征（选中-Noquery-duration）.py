...
def get_dataset_name(M, efC, efS, num_queries, ds_name, k, logint):
    return f"/home/extra_home/lxx23125236/ali/DARTH-main-data/data/et_training_data/early-stop-training/{ds_name}/k{k}/efS{efS}_qs{num_queries}_new.txt"
        ...
        for s in training_queries_num:
            ...
            for li in all_li:
                ...
                for selected_features in feature_classes:
                    ...
                    # 构造保存路径（和模型同名更清晰）
                    fi_file = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/feature_importance/raet/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_Noquery_duration.csv"
                    # 保存到文件
                    os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                    feature_importances.to_csv(fi_file, index=False)
                    print(f"特征重要性保存到: {fi_file}")

                    model_file = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/predictor_models/raet/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_Noquery_duration.txt"
...
def main():
    ...
    for ds_name in all_datasets:
        ...
        out_path = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/predictor_models/raet/k_50_{ds_name}_qs10000.json"
