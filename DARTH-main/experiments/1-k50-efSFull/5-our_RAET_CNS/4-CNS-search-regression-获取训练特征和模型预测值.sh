...
do
    ...
    do
        ...
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/${ds}/k${k}/train_data
          ...
          if true; then
            ...
            do
                ...
                ${BUILD_DIR}/hnsw-test/hnsw_test \
                  ...
                  --output $RESULTS_DIRECTORY/raet-cns-regression_CNSFull/${ds}/k${k}/train_data/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  --mode ${mode} \
                  --index-filepath ${INDEX_DIRECTORY}/${ds}/${ds}.M${M}.efC${efC}.index \
                  --dataset-dir-prefix ${DATASET_DIRECTORY} \
                  --target-recall ${target_recall} \
                  --query-type training \
                  --predictor-model-path ${MODEL_DIRECTORY}/${ds}/k${k}/efS${efS}_s${train_queries}_nestim${n_estim}_all_feats_${target_recall}_CNSFull.txt
