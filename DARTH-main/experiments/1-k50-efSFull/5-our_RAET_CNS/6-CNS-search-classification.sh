...
MODEL_DIRECTORY=/root/DARTH-main-data/predictor_models/raet-cns-classification
...
dataset_params=(
  ...
  "GIST1M 32 500 1000 1 1000"  #放弃二分类，占比CNS=k占比太小，二分类模型作用过小
  "DEEP10M 32 500 750 1 10000"
)

k_values=(100)

#期望准确率 达到要求准确率时的最小CNS 和平均稳定次数参数
SIFT1M_tuples_k100=(
#  "0.95 500"
#  "0.96 500"
  "0.97 500"
...
mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-classification
...
do
    ...
    mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}
    ...
    do
        ...
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/times/
        ...
        for tuple in "${tuples[@]}"; do
          ...
          JSON_FILE=${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/efS${efS}_times_summary_tr${target_recall}.json
          echo "[]" > "$JSON_FILE"
          ...
          if true; then
            ...
            do
              out="$(
                ${BUILD_DIR}/hnsw-test/hnsw_test \
                  ...
                  --output $RESULTS_DIRECTORY/raet-cns-classification/${ds}/k${k}/times/efS${efS}_qs${sample}_tr${target_recall}_t${time}.txt \
                  --mode ${mode} \
                  --index-filepath ${INDEX_DIRECTORY}/${ds}/${ds}.M${M}.efC${efC}.index \
                  --dataset-dir-prefix ${DATASET_DIRECTORY} \
                  --target-recall ${target_recall} \
                  --query-type testing \
                  --predictor-model-path ${MODEL_DIRECTORY}/${ds}/k${k}/efS${efSFull}_s${train_queries}_nestim${n_estim}_all_feats_${target_recall}.txt
              )"
            ...
            python /root/DARTH-main/experiments/ALL_graphIndex_trainData/多次搜索取平均.py \
                  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/times/efS${efS}_qs${sample}_tr${target_recall} \
                  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  $experiment_times

            echo "多次搜索中每个查询点的平均结果，写入完成：${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt"

            python /root/DARTH-main/experiments/ALL_graphIndex_trainData/Raet-CNS-早停统计.py \
                  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  ${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}.json \
                  $target_recall

            echo "从平均搜索结果文件中，所有查询点的早停统计结果statistics，写入完成：${RESULTS_DIRECTORY}/raet-cns-classification/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}.json"
