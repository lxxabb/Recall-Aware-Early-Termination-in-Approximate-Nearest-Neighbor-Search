            ...
            for target_recall in target_recalls:
                ...
                for li in all_li:
                    ...
                    for selected_features in feature_classes:
                        ...
                        # 构造保存路径（和模型同名更清晰）
                        fi_file = f"/root/DARTH-main-data/feature_importance/raet-cns-regression_CNSFull/{ds_name}/k{k}/efS{CNS}_s{s}_nestim{n_estimators}_{selected_features}_{target_recall}_CNSFull.csv"
                        # 保存到文件
                        os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                        feature_importances.to_csv(fi_file, index=False)
                        print(f"特征重要性保存到: {fi_file}")

                        model_file = f"/root/DARTH-main-data/predictor_models/raet-cns-regression_CNSFull/{ds_name}/k{k}/efS{CNS}_s{s}_nestim{n_estimators}_{selected_features}_{target_recall}_CNSFull.txt"
