...
def get_label_dataset_name(ds_name, k,target_recall,num_queries,efS):
    ...
    return f"/home/extra_home/lxx23125236/ali/DARTH-main-data/data/et_training_data/raet-CNS-fea-training/{ds_name}/k{k}/efs{efS}_min_ef_per_query_s{num_queries}_{recall_str}.csv"
...
#余弦特征：
    ...
    for ds_name in all_datasets:
        ...
        with open(f"/home/extra_home/lxx23125236/ali/DARTH-main-data/predictor_models/raet-cns-regression/k50_{ds_name}_qs10000_log.json", "w") as f:
            ...
            for target_recall in target_recalls:
                ...
                for li in all_li:
                    ...
                    for selected_features in feature_classes:
                        ...
                        recall_str = f"{target_recall:.2f}"
                        # 构造保存路径（和模型同名更清晰）
                        fi_file = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/feature_importance/raet-cns-regression_CNSFull/{ds_name}/k{k}/efS{CNS}_s{s}_nestim{n_estimators}_{selected_features}_{recall_str}_CNSFull_quantile{quantile}_Noquery_Noske_Nodensty.csv"
                        # 保存到文件
                        os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                        feature_importances.to_csv(fi_file, index=False)
                        print(f"特征重要性保存到: {fi_file}")

                        model_file = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/predictor_models/raet-cns-regression_CNSFull/{ds_name}/k{k}/efS{CNS}_s{s}_nestim{n_estimators}_{selected_features}_{recall_str}_CNSFull_quantile{quantile}_Noquery_Noske_Nodensty.txt"
