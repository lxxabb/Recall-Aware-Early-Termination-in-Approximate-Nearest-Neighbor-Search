...
def evaluate_one_model(ds_name, tr):
    ...
    # 如果想把模型预测时间也算进去，用这个
    avg_predict_ms = predict_time * 1000 / len(pred)
    selector_time_with_predict = selector_time_ideal + avg_predict_ms
    selector_total_with_predict_s = selector_time_with_predict.sum() / 1000.0

    # ================== 6. 命令行输出 ==================
    print(f"Selector Accuracy: {accuracy:.4f} ({correct}/{len(pred)})")
    print("--- 时间比较如下： ---")
    print(f"选择器方法总时间(不含预测开销) (s): {selector_total_s:.2f}")
    print(f"选择器方法总时间(含预测开销) (s): {selector_total_with_predict_s:.4f}")
    ...
    return {
        ...
        "selector_total_with_predict_s": selector_total_with_predict_s,
...
def main():
    ...
    for item in all_results:
        print(
            ...
            f"selector+pred={item['selector_total_with_predict_s']:.4f}s | "
