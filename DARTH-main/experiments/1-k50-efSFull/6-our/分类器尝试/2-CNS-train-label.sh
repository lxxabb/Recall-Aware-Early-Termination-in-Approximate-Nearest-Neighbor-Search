...
MODEL_DIRECTORY=/home/extra_home/lxx23125236/ali/DARTH-main-data/predictor_models/raet-cns-regression_CNSFull
...
dataset_params=(
#  "SIFT1M 32 500 500 1 20000"
#  "GLOVE100 16 500 500 1 20000"
#  "GIST1M 32 500 1000 1 20000"
#  "DEEP10M 32 500 750 1 20000"
...
do
    ...
    mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}
    ...
    do
        ...
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/times/
        ...
        for tuple in "${tuples[@]}"; do
          ...
          JSON_FILE=${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/efS${efS}_times_summary_tr${target_recall}.json
          echo "[]" > "$JSON_FILE"
          ...
          if true; then
            ...
            do
              out="$(
                ${BUILD_DIR}/hnsw-test/hnsw_test \
                  ...
                  --output $RESULTS_DIRECTORY/raet-cns-regression_CNSFull/train/${ds}/k${k}/times/efS${efS}_qs${sample}_tr${target_recall}_t${time}.txt \
                  --mode ${mode} \
            ...
            python /home/extra_home/lxx23125236/ali/DARTH-main/experiments/0-ALL_graphIndex_trainData/多次搜索取平均.py \
                  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/times/efS${efS}_qs${sample}_tr${target_recall} \
                  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  $experiment_times

            echo "多次搜索中每个查询点的平均结果，写入完成：${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt"

            python /home/extra_home/lxx23125236/ali/DARTH-main/experiments/0-ALL_graphIndex_trainData/Raet-CNS-早停统计.py \
                  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  ${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}.json \
                  $target_recall

            echo "从平均搜索结果文件中，所有查询点的早停统计结果statistics，写入完成：${RESULTS_DIRECTORY}/raet-cns-regression_CNSFull/train/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}.json"
