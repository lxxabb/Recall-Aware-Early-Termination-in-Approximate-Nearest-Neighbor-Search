...
model_file = f"{model_input_dir}/efS{efS}_qs10000_0.96_query_vec_model.txt"
...
print("\n--- 时间比较如下： ---")
print(f"理想情况选择器方法总时间(s): {total_seconds(selector_time_ideal.mean(), total_queries):.2f}")
print(f"最优情况选择器方法总时间(s): {total_seconds(optimal_time.mean(), total_queries):.2f}")
print(f"召回率方法总时间(s): {total_seconds(T_recall.mean(), total_queries):.2f}")
print(f"CNS方法总时间(s): {total_seconds(T_cns.mean(), total_queries):.2f}")