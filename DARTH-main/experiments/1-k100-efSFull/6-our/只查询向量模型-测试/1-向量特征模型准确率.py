...
TR_LIST = [0.95, 0.96, 0.97, 0.98, 0.99]
...
def evaluate_one_model(ds_name, tr):
    ...
    model_file = (
        f"{model_input_dir}/efS{efS}_qs10000_{tr:.2f}_query_vec_model.txt"
    )

    recall_label_file = (
        f"{data_dir}/result/raet/{ds_name}/k{k}/"
        f"efS{efS}_qs{query_num}_tr{tr:.2f}.txt"
    )
    cns_label_file = (
        f"{data_dir}/result/raet-cns-regression_CNSFull/{ds_name}/k{k}/"
        f"efS{efS}_qs{query_num}_tr{tr:.2f}.txt"
    )
    ...
    # [新增] 计算 F1 Score
    # binary: 默认模式，关注标签为 1 的类 (Recall更快)
    f1_binary = metrics.f1_score(best, pred)
    # macro: 计算两个类(0和1)的F1然后取平均，适合平衡数据集
    f1_macro = metrics.f1_score(best, pred, average='macro')

    # ================== 5. 时间分析 ==================
    selector_time_ideal = np.where(pred == 0, T_cns, T_recall)
    optimal_time = np.minimum(T_recall, T_cns)
    # [新增] 3. 计算每个查询的额外时间代价 (Delta t)
    # 公式: Delta t = t_selected - t_optimal
    extra_time_per_query = selector_time_ideal - optimal_time

    # [新增] 4. 计算平均额外时间代价 (Delta t_bar)
    # 注意：这里直接求均值，单位是 ms
    avg_extra_time_ms = np.mean(extra_time_per_query)
    ...
    # [新增] 打印 F1 分数
    print(f"Selector F1 (Binary): {f1_binary:.4f}")
    print(f"Selector F1 (Macro):  {f1_macro:.4f}")
    # [新增] 打印额外时间代价
    print("-" * 30)
    print(f"平均额外时间代价 (ms): {avg_extra_time_ms:.4f}")
    print("--- 时间比较如下： ---")
    ...
    return {
        ...
        "f1_binary": f1_binary,
        "f1_macro": f1_macro,
        "avg_extra_time_ms": avg_extra_time_ms,  # [新增]
