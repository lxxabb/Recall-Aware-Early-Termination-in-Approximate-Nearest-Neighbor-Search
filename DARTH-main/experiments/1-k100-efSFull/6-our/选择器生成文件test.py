...
def evaluate_one_model(ds_name, tr):
    ...
    recall_label_file = (
        f"/home/extra_home/lxx23125236/ali/DARTH-main-data/result/ali上时结果/hnsw-k00预测准确率方法/"
        f"efS{efS}_qs{query_num}_tr{tr:.2f}.txt"
    )
    cns_label_file = (
        f"/home/extra_home/lxx23125236/ali/DARTH-main-data/result/ali上时结果/hnsw-k00预测CNS方法/"
        f"efS{efS}_qs{query_num}_tr{tr:.2f}.txt"
    )
    ...
    # 确保 qid 对齐
    assert "qid" in recall_df.columns and "qid" in cns_df.columns, "qid 列不存在"
    assert recall_df["qid"].equals(cns_df["qid"]), "recall_df 和 cns_df 的 qid 顺序不一致"

    # 0: CNS更快, 1: Recall更快
    best = np.where(T_cns <= T_recall, 0, 1)

    # ================== 3.5 根据模型预测结果保存选择后的文件 ==================
    output_dir = f"{data_dir}/result/our_selected/{ds_name}/k{k}"
    os.makedirs(output_dir, exist_ok=True)

    selected_output_file = (
        f"/home/extra_home/lxx23125236/ali/DARTH-main-data/result/ali上时结果/选择器生成结果/efS{efS}_qs{query_num}_tr{tr:.2f}_selected.txt"
    )
    ...
    print(f"Selected file saved to: {selected_output_file}")
