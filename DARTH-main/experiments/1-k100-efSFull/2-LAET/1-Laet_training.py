...
def get_dataset_name(M, efC, efS, query_num, ds_name, k, logint):
    return f"/home/extra_home/lxx23125236/ali/DARTH-main-data/data/et_training_data/early-stop-training/{ds_name}/k{k}/efS{efS}_qs{query_num}.txt"
...
def main():
    ...
    for k in [100]:
        ...
        for ds_name in ["T2I1M"]:
            ...
            # 构造保存路径（和模型同名更清晰）
            fi_file = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/feature_importance/laet-our/{ds_name}/k{k}/efS{efS}_s{s}_f{F}.csv"
            # 保存到文件
            os.makedirs(os.path.dirname(fi_file), exist_ok=True)
            feature_importances.to_csv(fi_file, index=False)
            print(f"特征重要性保存到: {fi_file}")
        
            model_file = f"/home/extra_home/lxx23125236/ali/DARTH-main-data/predictor_models/laet-our/{ds_name}/k{k}/efS{efS}_s{s}_f{F}.txt"
