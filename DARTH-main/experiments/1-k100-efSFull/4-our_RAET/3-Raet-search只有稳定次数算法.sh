...
do
    ...
    do
        ...
        for tuple in "${tuples[@]}"; do
          ...
          JSON_FILE=${RESULTS_DIRECTORY}/raet/${ds}/k${k}/efS${efS}_times_summary_tr${target_recall}_only_stability.json
          echo "[]" > "$JSON_FILE"
          ...
          if true; then
            ...
            do
              out="$(
                ${BUILD_DIR}/hnsw-test/hnsw_test \
                  ...
                  --query-type testing \
                  --predictor-model-path ${MODEL_DIRECTORY}/${ds}/k${k}/efS${efSFull}_s${train_queries}_nestim${n_estim}_all_feats.txt
              )"
            ...
            python /root/DARTH-main/experiments/0-ALL_graphIndex_trainData/多次搜索取平均.py \
                  ...
                  ${RESULTS_DIRECTORY}/raet/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}_${stability_times}_only_stability.txt \
                  $experiment_times

            echo "多次搜索中每个查询点的平均结果，写入完成：${RESULTS_DIRECTORY}/raet/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}_${stability_times}_only_stability.txt"

            python /root/DARTH-main/experiments/0-ALL_graphIndex_trainData/Darth-Raet-早停统计.py \
                  ${RESULTS_DIRECTORY}/raet/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}_${stability_times}_only_stability.txt \
                  ${RESULTS_DIRECTORY}/raet/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}_only_stability.json \
                  $target_recall

            echo "从平均搜索结果文件中，所有查询点的早停统计结果statistics，写入完成：${RESULTS_DIRECTORY}/raet/${ds}/k${k}/efS${efS}_statistics_tr${target_recall}_only_stability.json"
