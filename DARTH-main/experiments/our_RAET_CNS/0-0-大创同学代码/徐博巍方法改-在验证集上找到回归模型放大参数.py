        ...
        for target_recall in target_recalls:
            ...
            for li in all_li:
                ...
                for selected_features in feature_classes:
                    ...
                    prop_100 = np.mean(y_train == k)
                    prop_CNS = np.mean(y_train == CNS)
                    print(f"值为 100 的占比: {prop_100:.4f} ({prop_100 * 100:.2f}%)")
                    print(f"值为 CNS ({CNS}) 的占比: {prop_CNS:.4f} ({prop_CNS * 100:.2f}%)")
                    ...
                    prop_positive = np.mean(y_pred > 0)
                    print(f"y_pred 大于 0 的占比: {prop_positive:.4f} ({prop_positive * 100:.2f}%)")
                    val_a = k
                    val_b = CNS
                    ...
                    result = minimize_scalar(
                        ...
                        bounds=(0.1, CNS/k),
                        args=(y_pred, y_train, val_a, val_b),
                        method='bounded'
                    )
                    best_k = result.x
