...
MODEL_DIRECTORY=/root/DARTH-main-data/predictor_models/raet-cns-regression
...
mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression
...
do
    ...
    mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}
    ...
    do
        ...
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}
        mkdir -p  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/times/
        ...
        for tuple in "${tuples[@]}"; do
          ...
          JSON_FILE=${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/times_summary_tr${target_recall}.json
          echo "[]" > "$JSON_FILE"
          ...
          if true; then
            ...
            do
              out="$(
                ${BUILD_DIR}/hnsw-test/hnsw_test \
                  ...
                  --output $RESULTS_DIRECTORY/raet-cns-regression/${ds}/k${k}/times/efS${efS}_qs${sample}_tr${target_recall}_t${time}.txt \
                  --mode ${mode} \
                  --index-filepath ${INDEX_DIRECTORY}/${ds}/${ds}.M${M}.efC${efC}.index \
                  --dataset-dir-prefix ${DATASET_DIRECTORY} \
                  --target-recall ${target_recall} \
                  --query-type testing \
                  --predictor-model-path ${MODEL_DIRECTORY}/${ds}/k${k}/efS${efSFull}_s${train_queries}_nestim${n_estim}_all_feats_${target_recall}_JieDuan.txt
              )"
            ...
            python /root/DARTH-main/experiments/ALL_graphIndex_trainData/多次搜索取平均.py \
                  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/times/efS${efS}_qs${sample}_tr${target_recall} \
                  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  $experiment_times

            echo "多次搜索中每个查询点的平均结果，写入完成：${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt"

            python /root/DARTH-main/experiments/ALL_graphIndex_trainData/Raet-CNS-早停统计.py \
                  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}.txt \
                  ${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/statistics_tr${target_recall}.json \
                  $target_recall

            echo "从平均搜索结果文件中，所有查询点的早停统计结果statistics，写入完成：${RESULTS_DIRECTORY}/raet-cns-regression/${ds}/k${k}/statistics_tr${target_recall}.json"
