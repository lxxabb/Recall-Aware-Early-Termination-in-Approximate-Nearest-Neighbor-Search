...
do
    ...
    do
        ...
        for tuple in "${tuples[@]}"; do
          ...
          JSON_FILE=${RESULTS_DIRECTORY}/darth/${ds}/k${k}/times_summary_tr${target_recall}.json
          echo "[]" > "$JSON_FILE"
          ...
          if true; then
            ...
            python /root/DARTH-main/experiments/ALL_graphIndex_trainData/Darth-Raet-早停统计.py \
                  ${RESULTS_DIRECTORY}/darth/${ds}/k${k}/efS${efS}_qs${sample}_tr${target_recall}_ipi${initial_prediction_interval}_mpi${min_prediction_interval}.txt \
                  ${RESULTS_DIRECTORY}/darth/${ds}/k${k}/statistics_tr${target_recall}.json \
                  $target_recall

            echo "从平均搜索结果文件中，所有查询点的早停统计结果statistics，写入完成：${RESULTS_DIRECTORY}/darth/${ds}/k${k}/statistics_tr${target_recall}.json"
