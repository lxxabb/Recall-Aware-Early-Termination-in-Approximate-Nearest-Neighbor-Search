...
def get_dataset_name(M, efC, efS, num_queries, ds_name, k, logint):
    # return f"/home/extra_home/lxx23125236/AKNNS_experiment/DARTH-main/data/et_training_data/early-stop-training/{ds_name}/k{k}/M{M}_efC{efC}_efS{efS}_qs{num_queries}_li{logint}_visitedPoints_filtered_r0.9.txt"
    ...
    return f"/home/extra_home/lxx23125236/AKNNS_experiment/DARTH-main-data/data/et_training_data/early-stop-training/{ds_name}/k{k}/M{M}_efC{efC}_efS{efS}_qs{num_queries}_li{logint}_visitedPoints_degree_hop.txt"
...
def get_query_dims_df(data_df, d, ds_name, s):
    ...
    if ds_name == "SIFT1M":
        query_dims, dims_read = read_fvecs(f"/home/extra_home/lxx23125236/ann-data/sift/sift_learn.fvecs", limit=s)
    elif ds_name == "GIST1M":
...
def train_predictors(ds_name, training_queries_num, all_k_values, all_li, M, efC, efS, d, n_estimators, columns_to_load, feature_classes, model_conf):
    ...
    for k in all_k_values:
        ...
        for s in training_queries_num:
            ...
            per_query_dimensions_df, dimensions = get_query_dims_df(query_data_all, d, ds_name, s)
            #获取标签
            min_cns_file = f"/home/extra_home/lxx23125236/AKNNS_experiment/DARTH-main-data/data/et_training_data/early-stop-training/SIFT1M/k100/min_ef_per_query.csv"
            ...
            for li in all_li:
                ...
                # y_all = data_all["r"]
                y_all = data_all["qid"].map(lambda qid: min_cns_df.loc[qid, "min_efSearch"])
