...
def get_query_dims_df(qid_df, d, ds_name, s):
    ...
    # ---- 先读 query embedding ----
    if ds_name == "SIFT1M":
        query_dims, dims_read = read_fvecs("/home/extra_home/lxx23125236/ann-data/sift/sift_learn.fvecs", limit=s)
    elif ds_name == "GIST1M":
        query_dims, dims_read = read_fvecs("/data/mchatzakis/datasets/processed/GIST1M/learn.100K.fvecs", limit=s)
    elif ds_name == "GLOVE100":
        query_dims, dims_read = read_fvecs("/home/extra_home/lxx23125236/ann-data/glove-100/glove原本数据集/learn.100K.fvecs", limit=s)
        ...
        for s in training_queries_num:
            ...
            for li in all_li:
                ...
                for selected_features in feature_classes:
                    ...
                    input_features =  ["step", "dists", "inserts","visited_points","first_nn_dist", "nn_dist", "furthest_dist","avg_dist", "variance"]
                    ...
                    # 构造保存路径（和模型同名更清晰）
                    fi_file = f"/root/DARTH-main-data/feature_importance/{ds_name}/raet_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_feature_importance_删除查询点向量特征.csv"
                    ...
                    model_file = f"/root/DARTH-main-data/predictor_models/raet/{ds_name}_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_filtered_r0.9_删除查询点向量特征.txt"
