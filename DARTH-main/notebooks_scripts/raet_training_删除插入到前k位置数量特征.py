        ...
        for s in training_queries_num:
            ...
            for li in all_li:
                ...
                for selected_features in feature_classes:
                    ...
                    #删除插入前k位置距离特征
                    input_features =  ["step", "dists", "visited_points","first_nn_dist", "nn_dist", "furthest_dist","avg_dist", "variance"] + query_dims_features
                    ...
                    # 构造保存路径（和模型同名更清晰）
                    fi_file = f"/root/DARTH-main-data/feature_importance/{ds_name}/raet_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_feature_importance_删除插入到前k数量.csv"
                    ...
                    model_file = f"/root/DARTH-main-data/predictor_models/raet/{ds_name}_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_filtered_r0.9_删除插入到前k数量.txt"
