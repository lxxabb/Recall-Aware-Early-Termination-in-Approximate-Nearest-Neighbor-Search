        ...
        for s in training_queries_num:
            ...
            for li in all_li:
                ...
                for selected_features in feature_classes:
                    ...
                    # 构造保存路径（和模型同名更清晰）
                    fi_file = f"/root/DARTH-main-data/feature_importance/{ds_name}/raet_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_feature_importance_删除入口点距离.csv"
                    ...
                    model_file = f"/root/DARTH-main-data/predictor_models/raet/{ds_name}_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_filtered_r0.9_删除入口点距离.txt"
