...
#预测CNS特征：res中的平均跳数
CNS_feats = ["avg_hop"]
all_feats = qid_feat+ index_metric_feats + neighbor_distances_feats + neighbor_stats_feats+CNS_feats

columns_to_load = ["elaps_ms"] + all_feats + ["r", "feats_collect_time_ms"]

# model_conf = {
#     "raet-lightgbm": lgb.LGBMRegressor(objective='regression', random_state=SEED, n_estimators=n_estimators, verbose = -1),
# }
...
def get_validation_dataset_name(M, efC, efS, query_num, ds_name, k, logint): 
    ...
    return f"/home/extra_home/lxx23125236/AKNNS_experiment/DARTH-main-data/data/et_training_data/test_logging/{ds_name}/k{k}/M{M}_efC{efC}_efS{efS}_qs{query_num}_li{logint}_visitedPoints_degree_hop.txt"
...
for ds_name in all_datasets:
    ...
    for k in tqdm(all_k_values, desc=f"k values for {ds_name}"):
        ...
        validation_data_dd = dd.read_csv(get_validation_dataset_name(M, efC, efS, 10000, ds_name, k, 1), usecols=columns_to_load)
        validation_data_df = validation_data_dd.compute()

        #获取标签
        min_cns_file = f"/home/extra_home/lxx23125236/AKNNS_experiment/DARTH-main-data/data/et_training_data/test_logging/SIFT1M/k100/min_ef_per_query_testing.csv"
        if not os.path.exists(min_cns_file):
            raise FileNotFoundError(f"Min CNS file not found: {min_cns_file}")

        min_cns_df = pd.read_csv(min_cns_file)
        min_cns_df.set_index("qid", inplace=True)  # 以 qid 作为索引
        validation_y_true = validation_data_df["qid"].map(lambda qid: min_cns_df.loc[qid, "min_efSearch"])

        per_validation_dimensions_df, dimensions = get_validation_dims_df(validation_data_df, d, ds_name, 10000)
        ...
        for s in training_queries_num:
            ...
            for li in all_lis:
                ...
                for selected_features in feature_classes.keys():
                    ...
                    model_file = f"/home/extra_home/lxx23125236/AKNNS_experiment/DARTH-main-data/predictor_models/raet/{ds_name}_M{M}_efC{efC}_efS{efS}_s{s}_k{k}_nestim{n_estimators}_li{li}_{selected_features}_visitedPoints_degree_hop.txt"
                    ...
                    input_features =  ["step", "dists", "inserts","visited_points","first_nn_dist", "nn_dist", "furthest_dist","avg_dist", "variance", "avg_hop"] + query_dims_features
                    ...
                    # "mse": 1048.4154241678864,
                    # "mae": 18.32530178295977,
                    # "r2": 0.4709931739627107,
                    # "p99": 132.25240154008435,
                    # "p1": 0.05451774836843982
