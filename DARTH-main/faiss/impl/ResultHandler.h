...
struct HeapBlockResultHandler : BlockResultHandler<C> {
     ...
     */
    //SingleResultHandler 是负责处理每个 query 的结果的对象。1、初始化堆  2、搜索过程中维护 top-k  3、搜索结束后排序（关键点！）
    struct SingleResultHandler : ResultHandler<C> {
        ...
        /// begin results for query # i
        //切换到第 i 个 query 的结果数组（每个 query 有 k 个槽位）
        // 调用 heap_heapify 把数组变成合法的最大堆
        // 堆顶 heap_dis[0] 是当前最大距离（也就是“最差的结果”）
        // 这个 threshold 用来判断新的候选是否进入 top-k。
        void begin(size_t i) {
        ...
        //只要新结果 比堆顶更好，就进入堆.搜索中用堆维持 top-k，不排序
        // 用 heap_replace_top 维持堆结构
        // 更新阈值 threshold
        /// add one result for query i
        bool add_result(T dis, TI idx) final {
        ...
        /// series of results for query i is done查询 i 的结果序列已完成
        //最终排序！把堆中的元素全部弹出，写回数组，并按距离从小到大排序。
        void end() {
