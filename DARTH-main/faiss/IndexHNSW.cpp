 ...
 **************************************************************/
    ...
    namespace {
        ...
        void hnsw_search_declarative_recall_data_generation(
            ...
            //n1：visited nodes（visited table count）number of vectors searched
            //n2：进入候选队列次数 number of queries for which the candidate list is exhasted 候选列表已用尽的查询数量
            //ndis：距离计算次数
            size_t n1 = 0, n2 = 0, ndis = 0;
...
#pragma omp for reduction(+ : n1, n2, ndis) schedule(guided)
        ...
        template<class BlockResultHandler>
        void hnsw_search_RAET_CNS_data_generation(
                const IndexHNSW *index,
                idx_t n,
                const float *x,
                BlockResultHandler &bres,
...
#pragma omp for reduction(+ : n1, n2, ndis) schedule(guided)
                    for (idx_t i = i0; i < i1; i++) {
                        ...
                        idx_t query_id_for_table = i - i0;
                        HNSWStats stats = hnsw.search_RAET_CNS_data_generation(
                                *dis,
                                res,
                                vt,
                                i,
        ...
        template<class BlockResultHandler>
        void hnsw_search_RAET_Select_data_generation(
                const IndexHNSW *index,
                idx_t n,
                const float *x,
                BlockResultHandler &bres,
...
#pragma omp for reduction(+ : n1, n2, ndis) schedule(guided)
                    for (idx_t i = i0; i < i1; i++) {
                        ...
                        idx_t query_id_for_table = i - i0;
                        HNSWStats stats = hnsw.search_RAET_Select_data_generation(
                                *dis,
                                res,
                                vt,
                                i,
        ...
        void hnsw_search_DARTH(
            ...
            //这估计一次搜索所需的工作量，用于决定多大 batch 后检查一次用户中断（Ctrl+C）。
            idx_t check_period = InterruptCallback::get_period_hint(
                    hnsw.max_level * index->d * efSearch);
            ...
            for (idx_t i0 = 0; i0 < n; i0 += check_period) {
                ...
                // #pragma omp parallel
                {
                    //VisitedTable 是 HNSW 搜索过程的访问标记表，用于避免重复访问节点。
                    VisitedTable vt(index->ntotal);
                    typename BlockResultHandler::SingleResultHandler res(bres);

                    //创建一个用于：设置 query 计算 query 与任意数据点的 L2 或 Inner Product 的距离
                    std::unique_ptr<DistanceComputer> dis(
                            storage_distance_computer(index->storage));
            ...
            std::cout << "Search time: " << elapsed << " seconds" << std::endl;
            hnsw_stats.combine({n1, n2, ndis});
        }

        template<class BlockResultHandler>
        void hnsw_search_RAET(
                const IndexHNSW *index,
                idx_t n,
                const float *x,
                BlockResultHandler &bres,
            ...
            for (idx_t i0 = 0; i0 < n; i0 += check_period) {
                ...
                // #pragma omp parallel
                {
                    ...
                    for (idx_t i = i0; i < i1; i++) {
                        ...
                        HNSWStats stats = hnsw.search_RAET(*dis, res, vt, i, raet_predictor, params);
                        n1 += stats.n1;
                        n2 += stats.n2;
                        ndis += stats.ndis;
                        res.end();
        ...
        void hnsw_search_RAET_CNS(
            ...
            for (idx_t i0 = 0; i0 < n; i0 += check_period) {
                ...
                // #pragma omp parallel
                {
                    ...
                    for (idx_t i = i0; i < i1; i++) {
                        ...
                        HNSWStats stats = hnsw.search_RAET_CNS(*dis, res, vt, i, raet_CNS_predictor, params);
                        n1 += stats.n1;
                        n2 += stats.n2;
                        ndis += stats.ndis;
                        res.end();
            ...
            double elapsed = raet_CNS_predictor.data_manager.elapsed_secs() - search_time_start;
            std::cout << "Search time: " << elapsed << " seconds" << std::endl;
            hnsw_stats.combine({n1, n2, ndis});
        }
        template<class BlockResultHandler>
        void hnsw_search_RAET_Select(
                const IndexHNSW *index,
                idx_t n,
                const float *x,
                BlockResultHandler &bres,
            ...
            for (idx_t i0 = 0; i0 < n; i0 += check_period) {
                ...
                // #pragma omp parallel
                {
                    ...
                    for (idx_t i = i0; i < i1; i++) {
                        ...
                        HNSWStats stats = hnsw.search_RAET_Select(*dis, res, vt, i, raet_predictor, params);
                        n1 += stats.n1;
                        n2 += stats.n2;
                        ndis += stats.ndis;
                        res.end();
        ...
        template<class BlockResultHandler>
        void hnsw_search_RAET_Query_Select(
                const IndexHNSW *index,
                idx_t n,
                const float *x,
                BlockResultHandler &bres,
                RAETPredictorHNSW raet_predictor,
                RAETCNSPredictorHNSW raet_CNS_predictor,
                RAETModelQuerySelectPredictorHNSW raet_selector,
                const SearchParameters *params_in) {
            FAISS_THROW_IF_NOT_MSG(
                    index->storage,
                    "No storage index, please use IndexHNSWFlat (or variants) "
            ...
            double search_time_start = raet_predictor.data_manager.elapsed_secs();
            double selector_pre_time_total=0;
            for (idx_t i0 = 0; i0 < n; i0 += check_period) {
                idx_t i1 = std::min(i0 + check_period, n);
                ...
                // #pragma omp parallel
                {
                    ...
                    for (idx_t i = i0; i < i1; i++) {
                        double pre_time_start = raet_predictor.data_manager.elapsed_secs();
                        //二分类模型选择器
                        int num_rows = 1;
                        int num_feats = raet_predictor.data_manager.d;
                        std::vector<double> feats(static_cast<size_t>(num_feats));
                        for (int j = 0; j < raet_predictor.data_manager.d; j++) {
                            feats[static_cast<size_t>(j)] =raet_predictor.data_manager.queries[i * raet_predictor.data_manager.d + j];
                        }
                        double out_result_selector[1];
                        long int out_len_selector;
                        int res_selector = LGBM_BoosterPredictForMatSingleRow(
                                raet_selector.booster_selector,
                                feats.data(),
                                C_API_DTYPE_FLOAT64,
                                num_feats,
                                1,
                        ...
                        selector_pre_time_total += raet_predictor.data_manager.elapsed_secs() - pre_time_start;

                        res.begin(i);
                        dis->set_query(x + i * index->d);
                        HNSWStats stats;
                        if(predict_select_recall_or_CNS==0){
                                stats = hnsw.search_RAET_CNS_Be_Used_By_Selector(*dis, res, vt, i, raet_predictor,raet_CNS_predictor, params);
                        }else{
                             stats = hnsw.search_RAET(*dis, res, vt, i, raet_predictor, params);
                        }
                        n1 += stats.n1;
                        n2 += stats.n2;
                        ndis += stats.ndis;
            ...
            double elapsed = raet_predictor.data_manager.elapsed_secs() - search_time_start;
            std::cout << "Search time: " << elapsed << " seconds" << std::endl;
            std::cout << "Selector model pre total time: " << selector_pre_time_total << " seconds" << std::endl;
            hnsw_stats.combine({n1, n2, ndis});
        }

        template<class BlockResultHandler>
        void hnsw_search_RAET_GET_STABILITY_TIMES(
                const IndexHNSW *index,
                idx_t n,
                const float *x,
                BlockResultHandler &bres,
            ...
            size_t n1 = 0, n2 = 0, ndis = 0;
            //平均稳定次数
            int avg_stability_counts=0;
            //最小稳定次数
            size_t min_stability_counts = std::numeric_limits<size_t>::max();
            //最大稳定次数
            size_t max_stability_counts = std::numeric_limits<size_t>::min();
            //平均已经达到多少次稳定
            int avg_has_achieve_stable_counts=0;
            //最少已经达到多少次稳定
            size_t min_has_achieve_stable_counts=std::numeric_limits<size_t>::max();
            //最大已经达到多少次稳定次数
            size_t max_has_achieve_stable_counts=std::numeric_limits<size_t>::min();
            //这估计一次搜索所需的工作量，用于决定多大 batch 后检查一次用户中断（Ctrl+C）。
            idx_t check_period = InterruptCallback::get_period_hint(
                    hnsw.max_level * index->d * efSearch);
            ...
            for (idx_t i0 = 0; i0 < n; i0 += check_period) {
                ...
                // #pragma omp parallel
                {
                    ...
                    for (idx_t i = i0; i < i1; i++) {
                        ...
                        HNSWStats_RAET stats = hnsw.search_RAET_GET_STABILITY_TIMES(*dis, res, vt, i, raet_predictor, stability_log_path, params);
                        n1 += stats.n1;
                        n2 += stats.n2;
                        ndis += stats.ndis;
                        avg_stability_counts+=stats.stability_counts;
                        min_stability_counts = std::min(min_stability_counts, stats.stability_counts);
                        max_stability_counts = std::max(max_stability_counts, stats.stability_counts);
                        avg_has_achieve_stable_counts+=stats.has_achieve_stable_counts;
                        min_has_achieve_stable_counts = std::min(min_has_achieve_stable_counts, stats.has_achieve_stable_counts);
                        max_has_achieve_stable_counts = std::max(max_has_achieve_stable_counts, stats.has_achieve_stable_counts);
                        res.end();
                    }
                }
                InterruptCallback::check();
            ...
            double elapsed = raet_predictor.data_manager.elapsed_secs() - search_time_start;
            std::cout << "平均稳定次数为" << avg_stability_counts/n <<  std::endl;
            std::cout << "最小稳定次数为" << min_stability_counts <<  std::endl;
            std::cout << "最大稳定次数为" << max_stability_counts <<  std::endl;
            std::cout << "平均已经达到多少次稳定" << avg_has_achieve_stable_counts/n <<  std::endl;
            std::cout << "最小已经达到多少次稳定" << min_has_achieve_stable_counts <<  std::endl;
            std::cout << "最大已经达到多少次稳定" << max_has_achieve_stable_counts <<  std::endl;
            hnsw_stats.combine({n1, n2, ndis});
        }
    ...
    void IndexHNSW::search_declarative_recall_data_generation(
        ...
        //构造一个 HeapBlockResultHandler, 用来存储 KNN 搜索结果
        using RH = HeapBlockResultHandler<HNSW::C>;
        RH bres(n, distances, labels, k);
    ...
    void IndexHNSW::search_RAET_CNS_data_generation(
            idx_t n,
            const float *x,
            idx_t k,
            float *distances,
        ...
        hnsw_search_RAET_CNS_data_generation(this, n, x, bres, data_collector, params_in);
    ...
    void IndexHNSW::search_RAET_Select_data_generation(
            idx_t n,
            const float *x,
            idx_t k,
            float *distances,
        ...
        using RH = HeapBlockResultHandler<HNSW::C>;
        RH bres(n, distances, labels, k);
        hnsw_search_RAET_Select_data_generation(this, n, x, bres, data_collector, params_in);
    ...
    void IndexHNSW::search_RAET_GET_STABILITY_TIMES(
            idx_t n,//查询向量数量
            const float *x,//查询向量集合
            idx_t k,
            float *distances,
        ...
        hnsw_search_RAET_GET_STABILITY_TIMES(this, n, x, bres, raet_predictor,stability_log_path,params_in);
    ...
    void IndexHNSW::search_RAET(
        ...
        hnsw_search_RAET(this, n, x, bres, raet_predictor, params_in);
    ...
    void IndexHNSW::search_RAET_CNS(
            idx_t n,//查询向量数量
            const float *x,//查询向量集合
            idx_t k,
            float *distances,
            idx_t *labels,
            RAETCNSPredictorHNSW raet_predictor,
            const SearchParameters *params_in) const {
        FAISS_THROW_IF_NOT(k > 0);
        ...
        hnsw_search_RAET_CNS(this, n, x, bres, raet_predictor, params_in);
    ...
    void IndexHNSW::search_RAET_Select(
            idx_t n,//查询向量数量
            const float *x,//查询向量集合
            idx_t k,
            float *distances,
        ...
        using RH = HeapBlockResultHandler<HNSW::C>;
        RH bres(n, distances, labels, k);
        hnsw_search_RAET_Select(this, n, x, bres, raet_predictor, params_in);
        if (is_similarity_metric(this->metric_type)) {
            // we need to revert the negated distances
            for (size_t i = 0; i < k * n; i++) {
                distances[i] = -distances[i];
    ...
    void IndexHNSW::search_RAET_Query_Select(
            idx_t n,//查询向量数量
            const float *x,//查询向量集合
            idx_t k,
            float *distances,
        ...
        using RH = HeapBlockResultHandler<HNSW::C>;
        RH bres(n, distances, labels, k);
        hnsw_search_RAET_Query_Select(this, n, x, bres, raet_predictor,raet_CNS_predictor,raet_selector, params_in);
        if (is_similarity_metric(this->metric_type)) {
            // we need to revert the negated distances
            for (size_t i = 0; i < k * n; i++) {
                distances[i] = -distances[i];
