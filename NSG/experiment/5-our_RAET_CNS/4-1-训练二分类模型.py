...
def get_dataset_name(M, efC, efS, num_queries, ds_name, k, logint):
    return f"/root/NSG-data/train_data/raet_CNS/{ds_name}/k{k}/efS{efS}_qs{num_queries}.txt"
def get_label_dataset_name(ds_name, k,target_recall,efS,num_queries):
    return f"/root/NSG-data/train_data/raet_CNS/{ds_name}/k{k}/label/efS{efS}_qs{num_queries}_{target_recall}.txt"
...
def main():
    ...
    neighbor_stats_feats = ["mean_distance", "variance_of_distances","avg_hop","avg_indegree","density","entry_query_dist_ratio","dist_distribution_entropy","skewness","kurtosis","energy"]
    ...
    for ds_name in all_datasets:
        ...
        with open(f"/root/DARTH-main-data/predictor_models/raet-cns-classification/{ds_name}_qs10000_log.json", "w") as f:
...
def train_predictors(ds_name, training_queries_num, all_k_values, all_li, M, efC, efS, d, n_estimators, columns_to_load, feature_classes, model_conf,target_recalls):
    ...
    for k in all_k_values:
        ...
        all_queries_dask = dd.read_csv(data_path, usecols=columns_to_load,dtype = {
            'step': 'int64',
            'distance_counts': 'int64',
            'inserts': 'int64',
            'first_nn_dist': 'float64',
            'nn_dist': 'float64',          # 🔥 关键
            'furthest_dist': 'float64',
            'mean_distance': 'float64',
            'variance_of_distances': 'float64',
            'avg_hop': 'float64',
            'avg_indegree': 'float64',
            'density': 'float64',
            'entry_query_dist_ratio': 'float64',
            'dist_distribution_entropy': 'float64',
            'skewness': 'float64',
            'kurtosis': 'float64',
            'energy': 'float64',
            'cosine_mean': 'float64',
            'cosine_variance': 'float64',
            'cosine_direction_entropy': 'float64',
        }
                                       )
        ...
        for s in training_queries_num:
            for target_recall in target_recalls:
                ...
                #获取标签
                label_path = get_label_dataset_name(ds_name, k,target_recall,efS,s)
                ...
                for li in all_li:
                    ...
                    y_all = data_all["qid"].map(lambda qid: min_cns_df.loc[qid, "L_min"])
                    # --- 生成二分类标签 ---
                    y_all = (y_all > k).astype(int)
                    ...
                    for selected_features in feature_classes:
                        ...
                        input_features =  ["step", "dists", "inserts","first_nn_dist", "nn_dist", "furthest_dist",
                                           "mean_distance", "variance_of_distances","avg_hop","avg_indegree","density","entry_query_dist_ratio","dist_distribution_entropy","skewness","kurtosis","energy",
                        ...
                        # 构造保存路径（和模型同名更清晰）
                        fi_file = f"/root/NSG-data/feature_importance/raet-cns-classification/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_{target_recall}.csv"
                        # 保存到文件
                        os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                        feature_importances.to_csv(fi_file, index=False)
                        print(f"特征重要性保存到: {fi_file}")

                        model_file = f"/root/NSG-data/predictor_models/raet-cns-classification/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_{target_recall}.txt"
