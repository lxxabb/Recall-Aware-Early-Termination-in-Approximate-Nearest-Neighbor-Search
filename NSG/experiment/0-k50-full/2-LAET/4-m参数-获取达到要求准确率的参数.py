import os
import time
import subprocess
import json
import pandas as pd
import numpy as np
import re

# LAET Tuning. Memory: False, training Size: 1000. Multiplier range: ['0.01', '0.06', '0.11', '0.16', '0.21', '0.26', '0.31', '0.36', '0.41', '0.46', '0.51', '0.56', '0.61', '0.66', '0.71', '0.76', '0.81', '0.86', '0.91', '0.96', '1.01', '1.06', '1.11', '1.16', '1.21', '1.26', '1.31', '1.36', '1.41', '1.46', '1.51', '1.56', '1.61', '1.66', '1.71', '1.76', '1.81', '1.86', '1.91', '1.96', '2.01', '2.06', '2.11', '2.16', '2.21', '2.26', '2.31', '2.36', '2.41', '2.46', '2.51', '2.56', '2.61', '2.66', '2.71', '2.76', '2.81', '2.86', '2.91', '2.96', '3.01', '3.06', '3.11', '3.16', '3.21', '3.26', '3.31', '3.36', '3.41', '3.46', '3.51', '3.56', '3.61', '3.66', '3.71', '3.76', '3.81', '3.86', '3.91', '3.96', '4.01', '4.06', '4.11', '4.16', '4.21', '4.26', '4.31', '4.36', '4.41', '4.46', '4.51', '4.56', '4.61', '4.66', '4.71', '4.76', '4.81', '4.86', '4.91', '4.96', '5.01']
# Dataset: T2I1M, k: 50, R_t: 0.9, Minimum Multiplier: 0.01, Average Recall: 0.9758, Tuning Time: 30.0855 ms, Total Experiments: 6
# Dataset: T2I1M, k: 50, R_t: 0.92, Minimum Multiplier: 0.01, Average Recall: 0.9758, Tuning Time: 35.52915 ms, Total Experiments: 6
# Dataset: T2I1M, k: 50, R_t: 0.94, Minimum Multiplier: 0.01, Average Recall: 0.9758, Tuning Time: 35.590849999999996 ms, Total Experiments: 6
# Dataset: T2I1M, k: 50, R_t: 0.96, Minimum Multiplier: 0.01, Average Recall: 0.9758, Tuning Time: 35.495650000000005 ms, Total Experiments: 6
# Dataset: T2I1M, k: 50, R_t: 0.98, Minimum Multiplier: 0.51, Average Recall: 0.98112, Tuning Time: 42.37885 ms, Total Experiments: 7

# Dataset: SIFT1M, k: 50, R_t: 0.9, Minimum Multiplier: 0.71, Average Recall: 0.908444, Tuning Time: 33.09092999999999 ms, Total Experiments: 7
# Dataset: SIFT1M, k: 50, R_t: 0.92, Minimum Multiplier: 0.76, Average Recall: 0.925728, Tuning Time: 29.9164 ms, Total Experiments: 6
# Dataset: SIFT1M, k: 50, R_t: 0.94, Minimum Multiplier: 0.86, Average Recall: 0.950054, Tuning Time: 33.90067 ms, Total Experiments: 7
# Dataset: SIFT1M, k: 50, R_t: 0.96, Minimum Multiplier: 0.96, Average Recall: 0.965179, Tuning Time: 35.73273 ms, Total Experiments: 7
# Dataset: SIFT1M, k: 50, R_t: 0.98, Minimum Multiplier: 1.16, Average Recall: 0.981708, Tuning Time: 37.406859999999995 ms, Total Experiments: 7
# Dataset: GIST1M, k: 50, R_t: 0.9, Minimum Multiplier: 0.61, Average Recall: 0.908621, Tuning Time: 38.60541 ms, Total Experiments: 6
# Dataset: GIST1M, k: 50, R_t: 0.92, Minimum Multiplier: 0.66, Average Recall: 0.921301, Tuning Time: 43.70151 ms, Total Experiments: 7
# Dataset: GIST1M, k: 50, R_t: 0.94, Minimum Multiplier: 0.81, Average Recall: 0.9463, Tuning Time: 44.39626 ms, Total Experiments: 7
# Dataset: GIST1M, k: 50, R_t: 0.96, Minimum Multiplier: 0.96, Average Recall: 0.96128, Tuning Time: 45.88625 ms, Total Experiments: 7
# Dataset: GIST1M, k: 50, R_t: 0.98, Minimum Multiplier: 1.31, Average Recall: 0.980419, Tuning Time: 53.603849999999994 ms, Total Experiments: 7
# Dataset: GLOVE100, k: 50, R_t: 0.9, Minimum Multiplier: 0.91, Average Recall: 0.924956, Tuning Time: 13.719400000000002 ms, Total Experiments: 6
# Dataset: GLOVE100, k: 50, R_t: 0.92, Minimum Multiplier: 0.91, Average Recall: 0.924956, Tuning Time: 13.63138 ms, Total Experiments: 6
# Dataset: GLOVE100, k: 50, R_t: 0.94, Minimum Multiplier: 0.96, Average Recall: 0.952122, Tuning Time: 15.81167 ms, Total Experiments: 7
# Dataset: GLOVE100, k: 50, R_t: 0.96, Minimum Multiplier: 1.01, Average Recall: 0.969317, Tuning Time: 15.91226 ms, Total Experiments: 7
# Dataset: GLOVE100, k: 50, R_t: 0.98, Minimum Multiplier: 1.11, Average Recall: 0.98413, Tuning Time: 16.38599 ms, Total Experiments: 7
# Dataset: DEEP10M, k: 50, R_t: 0.9, Minimum Multiplier: 0.71, Average Recall: 0.901083, Tuning Time: 50.730850000000004 ms, Total Experiments: 7
# Dataset: DEEP10M, k: 50, R_t: 0.92, Minimum Multiplier: 0.81, Average Recall: 0.926415, Tuning Time: 51.437250000000006 ms, Total Experiments: 7
# Dataset: DEEP10M, k: 50, R_t: 0.94, Minimum Multiplier: 0.91, Average Recall: 0.944814, Tuning Time: 47.37724 ms, Total Experiments: 6
# Dataset: DEEP10M, k: 50, R_t: 0.96, Minimum Multiplier: 1.06, Average Recall: 0.96466, Tuning Time: 56.332789999999996 ms, Total Experiments: 7
# Dataset: DEEP10M, k: 50, R_t: 0.98, Minimum Multiplier: 1.31, Average Recall: 0.982194, Tuning Time: 68.87472 ms, Total Experiments: 7


dataset_info = {
    "SIFT1M":{
        "M": 32,
        "efC": 500,
        "efS": 500,
        "d": 128,
        "F": 741 #固定 early termination search 预算
    },
    "GIST1M": {
        "M": 32,
        "efC": 500,
        "efS": 1000,
        "d": 960,
        "F": 2260,
    },
    "GLOVE100": {
        "M": 16,
        "efC": 500,
        "efS": 500,
        "d": 100,
        "F": 700,
    },
    "DEEP10M": {
        "M": 32,
        "efC": 500,
        "efS": 750,
        "d": 96,
        "F": 1118,
    },
    "T2I1M": {
        "M": 80,
        "efC": 1000,
        "efS": 1000,
        "d": 200,
        "F": 4000,
    },
}

# all_datasets = ["SIFT1M","GIST1M","GLOVE100","DEEP10M"]
all_datasets = ["T2I1M"]
all_k_values = [50]
all_recall_targets = [0.90, 0.92, 0.94, 0.96, 0.98]



def get_average_recall(training_df):
    return training_df["r"].mean()

def get_total_query_search_time(training_df):
    return training_df["elaps_ms"].sum()
def parse_time_and_recall(output: str):
    """
    从 C++ stdout 中提取：
    - search_time (double)
    - recall (float)
    """

    # 1️⃣ 提取搜索时间
    m_time = re.search(r"总搜索时间[:：]\s*([0-9.eE+-]+)", output)
    if not m_time:
        raise RuntimeError("Failed to parse search time")

    search_time = float(m_time.group(1))

    # 2️⃣ 提取准确率（支持中文 / 英文 / 带 K）
    m_recall = re.search(
        r"(?:\b\d+\s+)?NN\s+(?:Recall|准确率)\s*=\s*([0-9.eE+-]+)",
        output
    )
    if not m_recall:
        raise RuntimeError("Failed to parse recall")

    recall = float(m_recall.group(1))

    return recall, search_time

# LAET Automated Tuning
def run_laet(ds_name, M, efC, efS, s, F, k, m, target_recall, result_directory, output_log_file):
    mode = "Laet_test"
    os.makedirs(f"{result_directory}/{ds_name}/k{k}", exist_ok=True)
    m = float(m)   # 双保险
    command = [
        "/home/extra_home/lxx23125236/ali/NSG/cmake-build-lxx-release/tests/test_nsg_optimized_search",
        f"--dataset={ds_name}",
        f"--K={int(k)}",
        f"--L={int(efS)}",
        f"--mode={mode}",
        f"--query_type=train",
        f"--query_num={int(s)}",
        f"--target_recall={float(target_recall):.2f}",
        f"--laet_F={int(F)}",
        f"--laet_multiplier={m:.2f}",
        f"--model_path=/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/laet/{ds_name}/k{int(k)}/efS{int(efS)}_s{10000}_f{int(F)}.txt"
    ]

    result = subprocess.run(
        command,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
        check=True
    )

    output = result.stdout + "\n" + result.stderr

    recall, search_time = parse_time_and_recall(output)

    return recall, search_time

def get_laet_training_result_filename(ds_name, M, efC, efS, s, k, m, target_recall, result_directory):
    return f"{result_directory}/{ds_name}/k{k}/efS{efS}_qs{s}_tr{target_recall:.2f}_multiplier{m}.txt"

def find_min_multiplier(ds_name, M, efC, efS, s, F, k, recall_target, multipliers, min_m_idx, result_directory, output_log_file):
    """
    Find the minimum multiplier that achieves the target recall using binary search,
    starting from a given minimum index.
    """
    low, high = min_m_idx, len(multipliers) - 1
    best_m = None  # To store the minimum multiplier that satisfies the condition

    total_experiments = 0
    total_tuning_time = 0

    while low <= high:
        mid = (low + high) // 2
        m = multipliers[mid]

        # Run the algorithm and retrieve the results
        avg_recall, search_time = run_laet(ds_name, M, efC, efS, s, F, k, m, recall_target, result_directory, output_log_file)
        # training_df = pd.read_csv(get_laet_training_result_filename(ds_name, M, efC, efS, s, k, m, recall_target, result_directory))
        # avg_recall = get_average_recall(training_df)

        if avg_recall >= recall_target:
            best_m = m
            high = mid - 1 # Search for smaller m
        else:
            low = mid + 1  # Search for larger m

        total_experiments += 1
        total_tuning_time += search_time

    if best_m is None:
        print(f"[WARNING] -- Dataset: {ds_name}, k: {k}, R_t: {recall_target}, No suitable multiplier found. Using max multiplier.")
        best_m = multipliers[-1]

    return best_m, low, total_experiments, total_tuning_time

def laet_tuning(s, keep_m_idx_memory = False,):
    result_directory = "/home/extra_home/lxx23125236/ali/NSG-data/results/Laet-达到要求准确率的最小参数"
    output_log_filename = "/home/extra_home/lxx23125236/ali/NSG-data/results/Laet-达到要求准确率的最小参数/k50_laet_tuning.log"

    output_log_file = open(output_log_filename, "w")

    step = 0.05
    multiplier_range_upper_bound = 5.0
    multiplier_range_lower_bound = 0.01
    multipliers = np.arange(multiplier_range_lower_bound, multiplier_range_upper_bound + step, step)
    multipliers = [f"{m:.2f}" for m in multipliers]
    print(f"LAET Tuning. Memory: {keep_m_idx_memory}, training Size: {s}. Multiplier range: {multipliers}")

    tuning_results = {}
    for di, ds_name in enumerate(all_datasets):
        M = dataset_info[ds_name]["M"]
        F = dataset_info[ds_name]["F"]
        efS = dataset_info[ds_name]["efS"]
        efC = dataset_info[ds_name]["efC"]

        tuning_results[ds_name] = {}

        for k in all_k_values:
            tuning_results[ds_name][k] = {}
            min_m_idx = 0
            for recall_target in all_recall_targets:
                min_m, min_m_idx, total_experiments, total_tuning_time = find_min_multiplier(ds_name, M, efC, efS, s, F, k, recall_target, multipliers, min_m_idx, result_directory, output_log_file)

                avg_recall, search_time = run_laet(ds_name, M, efC, efS, s, F, k, min_m, recall_target, result_directory, output_log_file)

                print(f"Dataset: {ds_name}, k: {k}, R_t: {recall_target}, Minimum Multiplier: {min_m}, Average Recall: {avg_recall}, Tuning Time: {total_tuning_time} ms, Total Experiments: {total_experiments}")

                tuning_results[ds_name][k][recall_target] = {
                    "min_m": min_m,
                    "avg_recall": avg_recall,
                    "total_experiments": total_experiments,
                    "total_tuning_time": total_tuning_time
                }

                if not keep_m_idx_memory:
                    min_m_idx = 0

    filename = f"/home/extra_home/lxx23125236/ali/NSG-data/results/Laet-达到要求准确率的最小参数/k50_laet_tuning_results_memory{keep_m_idx_memory}_trainingSize{s}.json"
    with open(filename, "w") as f:
        json.dump(tuning_results, f, indent=4)

if __name__ == "__main__":
    for s in [1000]:
        laet_tuning(s, keep_m_idx_memory=False)
