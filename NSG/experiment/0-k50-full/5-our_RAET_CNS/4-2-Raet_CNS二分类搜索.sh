...
MODEL_DIRECTORY=/root/NSG-data/predictor_models/raet-cns-classification
experiment_times=1
mode=our_Raet_CNS_test_classification
k_values=(50)
train_queries=10000
n_estimators=100
#要求准确率，搜索时的CNS大小
          ...
          for time in $(seq 1 $experiment_times); do
            ...
            output=$(
              ${BUILD_DIR}/tests/test_nsg_optimized_search \
                ...
                --query_type=test \
                --query_num=${query_num} \
                --model_path_classification=${MODEL_DIRECTORY}/${ds}/k${k}/efS${efS_train}_s${train_queries}_nestim${n_estimators}_all_feats_${target_recall}_Noquery_Noske_Nodensty.txt \
