...
def get_dataset_name(M, efC, efS, num_queries, ds_name, k, logint):
    return f"/home/extra_home/lxx23125236/ali/NSG-data/train_data/raet_CNS/{ds_name}/k{k}/efS{efS}_qs{num_queries}.txt"
def get_label_dataset_name(ds_name, k,target_recall,efS,num_queries):
    recall_str = f"{target_recall:.2f}"
    return f"/home/extra_home/lxx23125236/ali/NSG-data/train_data/raet_CNS/{ds_name}/k{k}/label/efS{efS}_qs{num_queries}_{recall_str}.txt"
...
def main():
    ...
    all_datasets = ["T2I1M"]
    all_li = [1]
    # target_recalls = [0.90,0.92,0.94,0.96]
    for ds_name in all_datasets:
        ...
        with open(f"/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/raet-cns-regression/k100_{ds_name}_qs10000_log.json", "w") as f:
    ...
    for k in all_k_values:
        ...
        for s in training_queries_num:
            for target_recall in target_recalls:
                ...
                #获取标签
                label_path = get_label_dataset_name(ds_name, k,target_recall,efS,max_query_size)
                ...
                for li in all_li:
                    ...
                    y = data_all["qid"].map(lambda qid: min_cns_df.loc[qid, "L_min"])
                    ...
                    for selected_features in feature_classes:
                        ...
                        # #加权预测,使得能够达到0.90准确率
                        # weights = np.where(y_all == k, 0.01, 100.0)
                        ...
                        # 构造保存路径（和模型同名更清晰）
                        fi_file = f"/home/extra_home/lxx23125236/ali/NSG-data/feature_importance/raet-cns-regression/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_{recall_str}_quantile{quantile}_Noquery_Noske_Nodensty.csv"
                        # 保存到文件
                        os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                        feature_importances.to_csv(fi_file, index=False)
                        print(f"特征重要性保存到: {fi_file}")

                        model_file = f"/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/raet-cns-regression/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_{recall_str}_quantile{quantile}_Noquery_Noske_Nodensty.txt"
