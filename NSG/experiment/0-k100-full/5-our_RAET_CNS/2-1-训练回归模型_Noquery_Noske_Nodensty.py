...
def get_label_dataset_name(ds_name, k,target_recall,efS,num_queries):
    return f"/home/extra_home/lxx23125236/ali/NSG-data/train_data/raet_CNS/{ds_name}/k{k}/label/efS{efS}_qs{num_queries}_{target_recall}.txt"
            ...
            for target_recall in target_recalls:
                ...
                for li in all_li:
                    ...
                    for selected_features in feature_classes:
                        ...
                        # 构造保存路径（和模型同名更清晰）
                        fi_file = f"/home/extra_home/lxx23125236/ali/NSG-data/feature_importance/raet-cns-regression/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_{target_recall}_quantile{quantile}_Noquery_Noske_Nodensty.csv"
                        # 保存到文件
                        os.makedirs(os.path.dirname(fi_file), exist_ok=True)
                        feature_importances.to_csv(fi_file, index=False)
                        print(f"特征重要性保存到: {fi_file}")

                        model_file = f"/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/raet-cns-regression/{ds_name}/k{k}/efS{efS}_s{s}_nestim{n_estimators}_{selected_features}_{target_recall}_quantile{quantile}_Noquery_Noske_Nodensty.txt"
