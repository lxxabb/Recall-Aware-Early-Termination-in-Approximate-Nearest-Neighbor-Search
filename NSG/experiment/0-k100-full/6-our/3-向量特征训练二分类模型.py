...
def train_one_model(ds_name, tr):
    ...
    # ================== 输入文件路径 ==================
    recall_label_file = (
        f"{data_dir}/results/raet/train/{ds_name}/k{k}/"
        f"efS{efS}_s{s}_tr{tr:.2f}.txt"
    )
    cns_label_file = (
        f"{data_dir}/results/raet_CNS_regression/train/{ds_name}/k{k}/"
        f"efS{efS}_s{s}_tr{tr:.2f}.txt"
    )
    ...
    recall_time = recall_label_df["search_time"].values
    cns_time = cns_label_df["search_time"].values
    ...
    fi_file = (
        f"{fi_output_dir}/efS{efS}_s{s}_{tr:.2f}_query_vec_importance.csv"
    )
    ...
    # ================== 5. 保存模型 ==================
    model_file = (
        f"{model_output_dir}/efS{efS}_s{s}_{tr:.2f}_query_vec_model.txt"
    )
