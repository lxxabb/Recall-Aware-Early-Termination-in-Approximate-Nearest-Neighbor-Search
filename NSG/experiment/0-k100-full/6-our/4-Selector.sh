...
MODEL_DIRECTORY=/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/our
RECALL_MODEL_DIRECTORY=/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/raet
CNS_MODEL_DIRECTORY=/home/extra_home/lxx23125236/ali/NSG-data/predictor_models/raet-cns-regression
RESULTS_DIRECTORY=/home/extra_home/lxx23125236/ali/NSG-data/results/our
experiment_times=3
mode=our_Raet_CNS_selector
k_values=(100)
train_queries=10000
n_estimators=100
          ...
          for time in $(seq 1 $experiment_times); do
            ...
            output=$(
              ${BUILD_DIR}/tests/test_nsg_optimized_search \
                ...
                --model_path=${RECALL_MODEL_DIRECTORY}/${ds}/k${k}/efS${efS_train}_s${train_queries}_nestim${n_estimators}_all_feats_r_ge_0.90.txt \
                --model_path_classification=${CNS_MODEL_DIRECTORY}/${ds}/k${k}/efS${efS_train}_s${train_queries}_nestim${n_estimators}_all_feats_${target_recall}_quantile${quantile}_Noquery_Noske_Nodensty.txt \
                --out_put_path=${RESULTS_DIRECTORY}/${ds}/k${k}/efS${efS}_s${query_num}_tr${target_recall}.txt \
                --target_recall=${target_recall}
            )

            echo "$output"
            #model_path 是召回率预测模型路径
            #model_path_classification 是CNS预测模型路径
            # 提取搜索时间
