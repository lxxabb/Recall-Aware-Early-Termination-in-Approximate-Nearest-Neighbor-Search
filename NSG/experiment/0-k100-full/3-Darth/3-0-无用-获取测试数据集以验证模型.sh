...
k_values=(100)
TRAINING_DATA_DIRECTORY=/root/NSG-data/test_data
...
do
    ...
    for k in "${k_values[@]}"
    do
      mkdir -p ${TRAINING_DATA_DIRECTORY}/Laet_Darth_test_data/${ds}/k${k}
        echo ""
        echo "--------------- k=${k} ---------------"
        echo ""
            ${BUILD_DIR}/tests/test_nsg_optimized_search \
                ...
                --query_type=test \
                --query_num=${query_num} \
                --out_put_path=${TRAINING_DATA_DIRECTORY}/Laet_Darth_test_data/${ds}/k${k}/efS${efS}_qs${query_num}.txt
        echo "Laet_Darth方法，在${ds}测试集 k=${k}收集Laet_Darth特征完毕"
    done
    echo ""
    echo ""
done
