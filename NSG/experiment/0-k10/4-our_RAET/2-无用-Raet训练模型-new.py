...
def main(K: int, dim: int, feat_csv: str, learn_fvecs: str, model_out: str):
    ...
    k = K
    columns = ["qid", "step", "dists", "inserts",
               "visited_points", "nn_dist",
               "furthest_dist", "mean_distance", "variance_of_distances","r"]
    data = pd.read_csv(feat_csv,usecols=columns)
    ...
    label = data["r"]
    selected = ["qid","step", "dists", "inserts",
                "visited_points", "nn_dist",
                "furthest_dist", "mean_distance", "variance_of_distances"]
    feature_important_name=["step", "dists", "inserts","visited_points", "nn_dist","furthest_dist","mean_distance","variance_of_distances"]
    ...
    del learn_mat
    all_data = (selected_data.set_index('qid')
    ...
    model_conf = {
        "raet": lgb.LGBMRegressor(
            objective="regression",
            random_state=42,
            n_estimators=200,   # 等价 num_boost_round
