...
def main():
    ...
    for k in [10]:
        for ds_name in ["SIFT1M","GIST1M","DEEP10M"]:
            ...
            # 构造保存路径（和模型同名更清晰）
            fi_file = f"/root/NSG-data/feature_importance/laet/{ds_name}/k{k}/efS{efS}_s{s}_f{F}.csv"
            # 保存到文件
            os.makedirs(os.path.dirname(fi_file), exist_ok=True)
            feature_importances.to_csv(fi_file, index=False)
            print(f"特征重要性保存到: {fi_file}")

            model_file = f"/root/NSG-data/predictor_models/laet/{ds_name}/k{k}/efS{efS}_s{s}_f{F}.txt"
