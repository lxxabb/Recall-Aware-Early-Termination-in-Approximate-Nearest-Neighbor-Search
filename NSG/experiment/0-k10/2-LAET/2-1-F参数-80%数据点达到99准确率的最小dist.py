...
def get_dataset_name(M, efC, efS, query_num, ds_name, k):
    return f"/root/NSG-data/train_data/Laet_Darth_train_data/{ds_name}/k{k}/efS{efS}_qs{query_num}.txt"
...
def main():
    ...
    for k in [10]:
        for ds_name in ["SIFT1M","GIST1M","DEEP10M"]:
            ...
            # 只保留 target 列
            dists = per_query_input_feats_df["dists_for_max_recall"].astype(float)

            # 80% 分位数（使 80% query 能达到 recall≈0.99）
            dist_80 = np.quantile(dists, 0.8)

            print(f"[{ds_name}] 80% queries reach recall≥0.80 with dist ≤ {dist_80:.2f}")
